...
func (v *VRGInstance) undoPVCFinalizersAndPVRetentionForVGR(vrNamespacedName types.NamespacedName,
	...
	if err := v.deleteVGR(vrNamespacedName, log); err != nil {
		log.Info("Requeuing due to failure in finalizing VolumeReplication resource",
			"errorValue", err)
...
func (v *VRGInstance) processVGRAsSecondary(vrNamespacedName types.NamespacedName,
	...
	if v.instance.Spec.Sync != nil {
		...
		for idx := range pvcs {
			...
			v.updatePVCDataProtectedCondition(pvcNSName.Namespace, pvcNSName.Name, VRGConditionReasonDataProtected,
				msg)
			v.updatePVCLastSyncCounters(pvcNSName.Namespace, pvcNSName.Name, nil)
		}
